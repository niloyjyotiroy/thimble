...
if (page_title.indexOf('Наперстки') !== -1 || page_title.indexOf('Thimbles') !== -1 ) {
    window.onload = function(){
		...
		document.onclick = ({target}) => {
			...
			if (parent.className == 't-bets-btn');
			{
				if (ball_numb == 1)
...
function makeid() 
{
	...
	else
	{
		...
		$("#id_dop_str_title_123").text("Выберите напёрсток "+glob_row);
	}
}
...
function makeid1() 
{
	...
	else
	{
		...
		var html_0 = $("#id_dop_str_title").html();
		$("#id_dop_str_title").html(html_0+"<div style=\"color:white\">Выберите наперсток <strong style=\"color:white\">"+glob_row1+"</strong></div>");
	}
}